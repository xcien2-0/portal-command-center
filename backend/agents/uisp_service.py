...
_uisp_status: dict = {"ok": True, "error": None, "last_checked": None}
...
async def _get(path: str) -> Any:
    ...
    async with httpx.AsyncClient(verify=False, timeout=15) as client:
        ...
        _uisp_status["ok"] = True
        _uisp_status["error"] = None
        _uisp_status["last_checked"] = time.time()
        return data
...
async def get_topology_geo() -> list[dict]:
    ...
    """
    import asyncio, os, httpx as _httpx, time
    ...
    try:
        ...
        _uisp_status["ok"] = False
        _uisp_status["error"] = str(e)
        _uisp_status["last_checked"] = time.time()
        logger.warning(f"UISP topología no disponible: {e}")
        sites_coords, raw_links = {}, []
