...
// ── Tabs ──────────────────────────────────────────────────────────────────────
type Tab = 'dashboard' | 'leaderboard' | 'cursos';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
        <KPICard label="Técnicos en Odoo"  value={tecnicos.length}   sub="Con cuenta activa" />
...
// ── Leaderboard Tab ───────────────────────────────────────────────────────────
function TabLeaderboard({ tecnicos }: { tecnicos: Tecnico[] }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
        ...
        <span style={{ display: 'flex', alignItems: 'center', fontSize: 12, color: DIM }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.06)', background: '#0d1117' }}>
              {['#', 'Técnico', 'Nivel', 'Progreso', 'Cursos', 'Avance'].map(h => (
          ...
          <tbody>
            {slice.map((t) => (
              ...
              >
                ...
                <td style={{ padding: '12px 16px', fontSize: 13, color: DIM }}>{t.cursos}</td>
...
export default function AcademiaGerencial({ theme }: Props) {
  ...
  const tecnicos = useMemo(() => buildTecnicoList(cursos), [cursos]);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 0, height: '100%' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          ...
          <p style={{ margin: '4px 0 0', fontSize: 13, color: DIM }}>
            Vista gerencial · {tecnicos.length} técnicos · {cursos.filter(c => c.published).length} cursos activos
          </p>
        </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        ...
        {tab === 'leaderboard' && <TabLeaderboard tecnicos={tecnicos} />}
