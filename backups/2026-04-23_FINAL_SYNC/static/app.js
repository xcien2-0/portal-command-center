...
async function startDiagnostic() {
    // Mostrar formulario de nombre en el quiz-body antes de cargar preguntas
    ...
    document.getElementById('quiz-title').innerText = 'Examen de Diagnóstico 2026';
    document.getElementById('progress').style.width = '0%';
    document.getElementById('quiz-body').innerHTML = `
        <div style="text-align:center; padding: 2rem;">
            <h3 style="margin-bottom: 1rem;">¿Cuál es tu nombre completo?</h3>
            <input id="nombre-input" type="text" placeholder="Ej: Carlos Mendoza"
                style="width:100%; max-width:400px; padding:1rem; border-radius:10px;
            ...
            <br><br>
            <button class="primary-btn" onclick="confirmarNombre()" style="padding: 0.8rem 2rem;">Iniciar Examen ▶</button>
        </div>
    `;
    setTimeout(() => document.getElementById('nombre-input')?.focus(), 100);
}

async function confirmarNombre() {
    const nombre = document.getElementById('nombre-input')?.value?.trim();
    if (!nombre) {
        document.getElementById('nombre-input').style.borderColor = '#e74c3c';
        return;
    }
    ...
    document.getElementById('quiz-body').innerHTML = `
        <div style="text-align:center; padding:2rem;">
            <p style="color:#888;">Cargando tus preguntas...</p>
        </div>`;
    ...
    try {
        ...
        document.getElementById('quiz-body').innerHTML = `<p style="color:#e74c3c; text-align:center;">Error cargando el examen. Intenta de nuevo.</p>`;
    }
}
...
function showQuestion() {
    ...
    document.getElementById('quiz-title').innerText = quizData.titulo || 'Examen Xcien';
    document.getElementById('progress').style.width = `${((currentQuestionIndex + 1) / quizData.preguntas.length) * 100}%`;

    const body = document.getElementById('quiz-body');
    body.innerHTML = `
        <div class="question-box">
            <h3>Pregunta ${currentQuestionIndex + 1} de ${quizData.preguntas.length}: ${q.pregunta}</h3>
            <div class="options-list">
                ${q.opciones.map((opt, idx) => `
                    <button class="option-btn" id="opt-${idx}" onclick="selectOption(${idx})">${opt}</button>
                `).join('')}
            </div>
        </div>
    `;
