...
base_dir = os.path.expanduser("~/Desktop/Live status Xcien/src/pages/xcien2/sections")
holo_path = os.path.join(base_dir, "AcademiaHoloSection.tsx")
...
with open(holo_path, "w") as f:
    f.write(react_code)

print("AcademiaHoloSection.tsx created.")

# Apply the user request to move "holo" to "tokens" / "Certificación" group
import re

index_path = os.path.join(os.path.expanduser("~/Desktop/Live status Xcien/src/pages/xcien2"), "index.tsx")
with open(index_path, "r") as f:
    index_content = f.read()

# Make sure to update the label and group if it already exists from our previous patch
if "label: 'Examen Holo'" in index_content:
    index_content = index_content.replace(
        "label: 'Examen Holo',           icon: '🔮', group: 'Academia XCIEN'",
        "label: 'Certificación Holo',    icon: '🔮', group: 'Operaciones'"
    )

with open(index_path, "w") as f:
    f.write(index_content)
    
print("index.tsx updated with Certificación grouping.")
